...
class WindowMultiHeadAttention(nn.Module):
    ...
    def __self_attention(self,
        ...
        if mask is not None: 
            number_of_windows: int = mask.shape[1] if len(mask.shape) == 4 else mask.shape[0]#(batch):
            ...
            mask = mask.unsqueeze(2) if len(mask.shape) == 4 else mask.unsqueeze(1).unsqueeze(0)
            attention_map: torch.Tensor = attention_map + mask
...
class PatchScattering(nn.Module):
    def __init__(self,
                 in_channels: int) -> None:
        """
        Constructor method
        ...
        """
        # Call super constructor
        super(PatchScattering, self).__init__()
        # Init normalization
        self.normalization: nn.Module = nn.LayerNorm(normalized_shape=4 * in_channels)
        # Init linear mapping
        self.linear_mapping: nn.Module = nn.Linear(in_features=4 * in_channels, out_features=8 * in_channels,
                                                   bias=False)
        ...
        for index, (depth, number_of_head) in enumerate(zip(depths, number_of_heads)):
            self.stages.append(
                SwinTransformerStage(
                    ...
                    sequential_self_attention=sequential_self_attention,
                    use_deformable_block=use_deformable_block and (index == len(depths)-1) # only for the last
                ))
...
class PredSwinTransformerV2(nn.Module):
    """
    This class implements the Swin Transformer (Pred) without classification head.
    """
    ...
    def __init__(self,
                 ...
                 sequential_self_attention: bool = False,
                 use_deformable_block: bool = False,
                 flow_inputs: bool = False,
                 large_scale: bool =False,) -> None:
        ...
        """
        # Call super constructor
        super(PredSwinTransformerV2, self).__init__()
        # Save parameters
        self.patch_size: int = patch_size
        # Init patch embedding
        self.patch_embedding: nn.Module = PatchEmbedding(in_channels=in_channels, out_channels=embedding_channels,
                                                         patch_size=patch_size)
        self.flow_embedding: nn.Module = PatchEmbedding(in_channels=6 if large_scale else 2, out_channels=embedding_channels,
                                                         patch_size=patch_size)
        self.flow_inputs = flow_inputs
        # Compute patch resolution
        patch_resolution: Tuple[int, int] = (input_resolution[0] // patch_size, input_resolution[1] // patch_size)
        ...
        self.flow_stage = SwinTransformerStage(
                    in_channels=embedding_channels,
                    depth=depths[0],
                    downscale=False,
                    input_resolution=(patch_resolution[0], patch_resolution[1]),
                    number_of_heads=number_of_heads[0],
                    window_size=window_size,
                    ff_feature_ratio=ff_feature_ratio,
                    dropout=dropout,
                    dropout_attention=dropout_attention,
                    dropout_path=dropout_path[sum(depths[:0]):sum(depths[:0 + 1])], # only for the last
                )
    ...
    def forward(self, input: torch.Tensor, flow: torch.Tensor) -> List[torch.Tensor]:
        """
        Forward pass
        :param input: (torch.Tensor) Input tensor
        :return: (List[torch.Tensor]) List of features from each stage
        ...
        for stage in self.stages:
            output: torch.Tensor = stage(output)
            if i ==0 :
                flow = self.flow_stage(flow)
                output += flow
            features.append(output)
            i += 1
        return features, flow

# from HR_components import HR_converter

class PredHRSwinTransformerV2(nn.Module):
    """
    This class implements the Swin Transformer (Pred) without classification head.
    """
    ...
    def __init__(self,
        ...
        # Call super constructor
        super(PredHRSwinTransformerV2, self).__init__()
        # Save parameters
        self.patch_size: int = patch_size
        # Init patch embedding
        self.patch_embedding: nn.Module = PatchEmbedding(in_channels=in_channels, out_channels=embedding_channels,
        ...
        for idx in range(len(depths)):
            hr_stage = nn.ModuleList()
            for index, (depth, number_of_head) in enumerate(zip(depths[:idx+1], number_of_heads[:idx+1])):
                # print(embedding_channels * (2 ** max(index - 1, 0)))
                # print(patch_resolution[0] // (2 ** max(index - 1, 0)))
                hr_stage.append(
                    SwinTransformerStage(
                        in_channels=embedding_channels * (2 **index),
                        depth=depth,
                        downscale=False,
                        input_resolution=(patch_resolution[0] // (2 ** index),
                                        patch_resolution[1] // (2 ** index)),
                        number_of_heads=number_of_head,
                        window_size=window_size,
                        ff_feature_ratio=ff_feature_ratio,
                        dropout=dropout,
                        ...
                        use_checkpoint=use_checkpoint,
                        sequential_self_attention=sequential_self_attention,
                        use_deformable_block=False # only for the last
                    )
                    )
            self.stages.append(hr_stage)
        
        # HR connections:
        base_res = input_resolution[0] // patch_size
        self.HR_stages = nn.ModuleList()
        for l in range(len(self.stages)-1):
            from_res = [int(base_res//(2**i)) for i in range(l + 1)]
            from_dim = [int(embedding_channels*(2**i)) for i in range(l + 1)]
            to_res = [int(base_res//(2**i)) for i in range(l + 2)]
            # print(from_res, to_res, from_dim)
            self.HR_stages.append(HR_converter(from_res, to_res, from_dim))
    ...
    def forward(self, input: torch.Tensor) -> List[torch.Tensor]:
        ...
        features = [output]
        # Init list to store feature
        for index, hr_stage in enumerate(self.stages):
            hr_list = []
            for feat, swin_stage in zip(features, hr_stage):
                out = swin_stage(feat)
                # print(out.shape)
                hr_list.append(out)
            if index != len(self.stages) - 1:
                features = self.HR_stages[index](hr_list)
            else:
                features = hr_list     
...
def test_hr_swint():
    ...
    HR_Swin = PredHRSwinTransformerV2(
        in_channels=96,embedding_channels=96,
        depths=[2,2,2],input_resolution=(256,256),number_of_heads=[3, 6, 12],window_size=8
    )
    s = time()
    enc_inputs = torch.randn((1, 96, 256, 256))
    print(time()-s)
    outputs, _ = HR_Swin(enc_inputs)
    for o in outputs:
        print(o.shape)
