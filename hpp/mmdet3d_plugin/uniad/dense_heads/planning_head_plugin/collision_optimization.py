...
from casadi import DM, Opti, OptiSol, cos, diff, sin, sumsqr, vertcat, exp, fabs, sign, lt
...
class CollisionNonlinearOptimizer:
    ...
    def __init__(self, trajectory_len: int, dt: float, sigma, alpha_collision, obj_pixel_pos,
        pred_xy=None, pred_xy_mask=None, pred=False):
        """
        ...
        self.pred_xy_mask = pred_xy_mask
        ...
        self._init_optimization(pred)

    def _init_optimization(self, pred=False) -> None:
        """
    ...
    def _set_objective(self, pred=False) -> None:
        ...
        cost = cost_stage + cost_collision
        if pred:
            pred_cost = 0
            for t in range(len(self.pred_xy)):
                x, y = self.position_x[t], self.position_y[t]
                x_mask, y_mask =  self.pred_xy_mask[t][0], self.pred_xy_mask[t][1]
                for i in range(len(self.pred_xy[t])):
                    pred_x, pred_y = self.pred_xy[t][i][0], self.pred_xy[t][i][1]
                    if x_mask !=0:
                        pred_cost += alpha_collision * normalizer * exp(-((x - pred_x)**2 + (y - pred_y)**2)/2/self.sigma**2)
            cost += pred_cost
