...
class PlanningMetric(Metric):
    def __init__(
        ...
        self.add_state("inconsistency_rate", default=torch.zeros(self.n_future), dist_reduce_fx="sum")
    ...
    def evaluate_single_coll(self, traj, segmentation):
        ...
        trajs = trajs / self.dx
        trajs = trajs.cpu().numpy() + self.rc # (n_future, 32, 2)
    ...
    def update(self, trajs, gt_trajs, gt_trajs_mask, segmentation, org_trajs):
        '''
        trajs: torch.Tensor (B, n_future, 3)
        gt_trajs: torch.Tensor (B, n_future, 3)
        segmentation: torch.Tensor (B, n_future, 200, 200)
        ...
        org_trajs[..., 0] = - org_trajs[..., 0]
        orgobj_coll_sum, orgobj_box_coll_sum = self.evaluate_coll(org_trajs[:,:,:2], gt_trajs[:,:,:2], segmentation)

        sum_col = (obj_coll_sum + obj_box_coll_sum) > 0
        sum_org_not_col = (orgobj_coll_sum + orgobj_box_coll_sum) == 0
        incons_rate = torch.logical_and(sum_col, sum_org_not_col).long()

        self.current_state = dict(
            plan_errors=L2.sum(dim=0),
            plan_cols=sum_col.long()
        )

        self.obj_col += obj_coll_sum
        self.obj_box_col += obj_box_coll_sum
        self.inconsistency_rate += incons_rate
    ...
    def compute(self):
        return {
            ...
            'ICR': self.inconsistency_rate / self.total,
        }