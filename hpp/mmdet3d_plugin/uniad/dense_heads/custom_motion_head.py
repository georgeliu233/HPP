...
class CustomMotionHead(BaseMotionHead):
    """
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(CustomMotionHead, self).__init__()
        ...
        # self.rel_mode_embedding_layer = nn.Sequential(
    ...
    def forward_train(self,
        ...
        outs_motion['track_rel_pos'] = bbox_centers - bbox_centers[-1, None, :]
        outs_motion['track_rel_yaw'] = yaw - yaw[-1]
        ...
        def filter_vehicle_query(outs_motion, all_matched_idxes, gt_labels_3d, vehicle_id_list):
            ...
            outs_motion['traj_scores'] = outs_motion['traj_scores'][:, :, vehicle_mask>0]
            outs_motion['traj_preds'] = outs_motion['traj_preds'][:, :, vehicle_mask>0]
            # outs_motion['track_scores'] = outs_motion['track_scores'][:, vehicle_mask>0]
            outs_motion['track_rel_pos'] = outs_motion['track_rel_pos'][vehicle_mask>0]
            outs_motion['track_rel_yaw'] = outs_motion['track_rel_yaw'][vehicle_mask>0]
        ...
        outs_motion['traj_scores'] = outs_motion['all_traj_scores'][:, :, :-1]
        outs_motion['traj_preds'] = outs_motion['all_traj_preds'][:, :, :-1]
        outs_motion['track_rel_pos'] = outs_motion['track_rel_pos'][:-1]
        outs_motion['track_rel_yaw'] = outs_motion['track_rel_yaw'][:-1]
        
        outs_motion, all_matched_idxes = filter_vehicle_query(outs_motion, all_matched_idxes, gt_labels_3d, self.vehicle_id_list)
        assert outs_motion['traj_query'].shape[2] == outs_motion['traj_scores'].shape[2]
    ...
    def forward_test(self, bev_embed, outs_track={}, outs_seg={}):
        ...
        bboxes, scores, labels, bbox_index, mask = track_boxes[0]
        yaw = bboxes.yaw
        bbox_centers = bboxes.gravity_center[:, :2]
        outs_motion['track_scores'] = scores[None, :]
        outs_motion['track_rel_pos'] = bbox_centers - bbox_centers[-1, None, :]
        outs_motion['track_rel_yaw'] = yaw - yaw[-1]
        labels[-1] = 0
        def filter_vehicle_query(outs_motion, labels, vehicle_id_list):
            ...
            outs_motion['track_rel_pos'] = outs_motion['track_rel_pos'][vehicle_mask>0][:-1]
            outs_motion['track_rel_yaw'] = outs_motion['track_rel_yaw'][vehicle_mask>0][:-1]

            outs_motion['traj_scores'] = outs_motion['all_traj_scores'][:, :, vehicle_mask>0]
            outs_motion['traj_preds'] = outs_motion['all_traj_preds'][:, :, vehicle_mask>0]
            return outs_motion
        
        outs_motion = filter_vehicle_query(outs_motion, labels, self.vehicle_id_list)
        
        # filter sdc query
        # print(outs_motion['traj_query'].shape)
        ...
        outs_motion['traj_scores'] = outs_motion['traj_scores'][:, :, :-1]
        outs_motion['traj_preds'] = outs_motion['traj_preds'][:, :, :-1]
    ...
    def forward(self, 
        ...
        #last time use #scene_level_ego_norm !!!
        # last_scene_level_ego_norm = scene_level_offset_norm[..., -1, :] #[b, a, g, p, 2]
        # last_scene_level_ego_norm = self.group_mode_query_pos(track_bbox_results, last_scene_level_ego_norm)
        # b, num_agents, num_modes, d = last_scene_level_ego_norm.shape
        # last_scene_level_ego_norm = last_scene_level_ego_norm.unsqueeze(2).unsqueeze(2).expand(-1, -1, num_modes, num_agents, -1, -1) #[b, a, m, a, m, 2]
        # agent_rel_pos = rel_norm_points(self.get_mode_rel_pos(track_bbox_results, last_scene_level_ego_norm), self.pc_range)
        # last_scene_level_ego_norm = last_scene_level_ego_norm + agent_rel_pos[:, :, None, :, None, :]
        # last_scene_level_ego_norm = last_scene_level_ego_norm.reshape(b, num_agents*num_modes, num_agents*num_modes, d)
        
        # rel_mode_embedding = self.rel_mode_embedding_layer(pos2posemb2d(last_scene_level_ego_norm))
        ...
        inter_states, inter_references = self.motionformer(
            ...
            # rel_mode_embedding_layer=self.rel_mode_embedding_layer,
            # rel_mode_embedding=rel_mode_embedding,
            # agent_rel_pos=agent_rel_pos)
        )
        ...
        # print(outputs_traj_scores.shape,outputs_trajs.shape,inter_states.shape, track_query.shape)
        outs = {
    ...
    def get_mode_rel_pos(self, bbox_results, transformed_anchors):
        ...
        for i in range(batch_size):
            ...
            mode_pos.append(bbox_centers)
        mode_pos = torch.stack(mode_pos, dim=0) #[b, agent, 2]
        mode_rel_pos = mode_pos[:, :, None, :] - mode_pos[:, None, :, :]
        return mode_rel_pos
