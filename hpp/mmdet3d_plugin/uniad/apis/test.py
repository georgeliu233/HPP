...
def custom_multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False):
    ...
    if eval_occ:
        ...
        if flow_pred:
            for key in EVALUATION_RANGES.keys():
                iou_metrics[key+'flow_warp'] = IntersectionOverUnion(n_classes).cuda()
        ...
        if flow_pred:
            for key in EVALUATION_RANGES.keys():
                panoptic_metrics[key+'flow_warp'] = PanopticMetric(n_classes=n_classes, temporally_consistent=True).cuda()
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            if eval_planning:
                ...
                org_planning = result[0]['planning']['result_planning']['sdc_traj_org']
                planning_metrics(pred_sdc_traj[:, :6, :2], sdc_planning[0][0,:, :6, :2], sdc_planning_mask[0][0,:, :6, :2], segmentation[0][:, [1,2,3,4,5,6]],
                org_planning[:, :6, :2])
                result[0]['comp_plan_errors'] = planning_metrics.current_state['plan_errors'].detach().cpu().numpy()
                result[0]['comp_plan_col'] = planning_metrics.current_state['plan_cols'].detach().cpu().numpy()
            ...
            if eval_occ:
                ...
                if occ_to_eval and 'occ' in result[0].keys():
                    ...
                    if flow_pred:
                        for key, grid in EVALUATION_RANGES.items():
                            limits = slice(grid[0], grid[1])
                            iou_metrics[key+'flow_warp'](result[0]['occ']['seg_out_warp'][..., limits, limits].contiguous(),
                                            result[0]['occ']['seg_gt'][..., limits, limits].contiguous())
                            panoptic_metrics[key+'flow_warp'](result[0]['occ']['ins_seg_out_warp'][..., limits, limits].contiguous().detach(),
                                                    result[0]['occ']['ins_seg_gt'][..., limits, limits].contiguous())

            # with open(f"/home/users/ntu/haochen0/scratch/UniAD/occ_data/uniad/{result[0]['token']}.pkl", 'wb') as writer:
            #     pickle.dump(result[0]['occ']['seg_out'].detach().cpu().numpy(), writer)
            ...
            if os.environ.get('ENABLE_PLOT_MODE', None) is None:
                ...
                for k in ['seg_gt', 'ins_seg_gt', 'pred_ins_sigmoid', 'seg_out', 'ins_seg_out']:
                    if k in result[0]['occ']:
                        result[0]['occ'][k] = result[0]['occ'][k].detach().cpu().numpy()
    ...
    if eval_occ:
        ...
        for key, grid in EVALUATION_RANGES.items():
            ...
            if flow_pred:
                panoptic_scores = panoptic_metrics[key+'flow_warp'].compute()
                for panoptic_key, value in panoptic_scores.items():
                    occ_results[f'{panoptic_key}'+'flow_warp'] = occ_results.get(f'{panoptic_key}'+'flow_warp', []) + [100 * value[1].item()]
                panoptic_metrics[key+'flow_warp'].reset()

                iou_scores = iou_metrics[key+'flow_warp'].compute()
                occ_results['iou_flow_warp'] = occ_results.get('iou_flow_warp', []) + [100 * iou_scores[1].item()]
                iou_metrics[key+'flow_warp'].reset()
