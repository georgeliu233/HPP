...
class UniAD(UniADTrack):
    ...
    def forward_train(self,
        ...
        if self.with_occ_head:
            ...
            losses_occ, occ_dict = self.occ_head.forward_train(
                            bev_embed, 
                            outs_motion, 
        ...
        if self.with_planning_head:
            ...
            ego_lcf_feat = torch.tensor([can_bus[13], can_bus[14], can_bus[7], can_bus[8], can_bus[12]]).to(bev_embed.device)
            ego_lcf_feat = ego_lcf_feat[None]
            ...
            gt_sdc_fut_traj, gt_sdc_fut_traj_mask, occ_dict['bev_mask'], occ_dict['bev_target'], occ_dict['frame_mask'], ego_lcf_feat)
    ...
    def forward_test(self,
        ...
        track_center = self.motion_head._extract_tracking_centers(result_track[0]['track_bbox_results'], self.motion_head.pc_range)

        if self.max_agents >-1:
            dist = torch.linalg.norm(track_center, dim=-1)
            if dist.shape[-1] > self.max_agents:
                _,track_idx = torch.topk(dist, k=self.max_agents, largest=False, dim=-1)
                new_bbox_list = []
                for i in range(len(result_track[0]['track_bbox_results'])):
                    new_bbox_list.append([ele[track_idx[i]] for ele in result_track[0]['track_bbox_results'][i]])
                result_track[0]['track_bbox_results'] = deepcopy(new_bbox_list)
                b = len(result_track[0]['track_bbox_results'])
                #[n-agents, d]; [1, n_agent]; [1, n_agent, 2], [1, max_agent]
                # print(result_track[0]['track_query_embeddings'].shape, dist.shape, track_center.shape, track_idx.shape)
                result_track[0]['track_query_embeddings'] = result_track[0]['track_query_embeddings'][track_idx[0]]
                # result_track[0]['track_scores'] = result_track[0]['track_scores'][torch.arange(b), track_idx]
        ...
        if self.with_planning_head:
            ...
            result_planning = self.planning_head.forward_test(bev_embed, outs_motion, outs_occ, command, ego_lcf_feat)
