...
class NuScenesTraj(object):
    ...
    def get_sdc_planning_label(self, sample_token, his_ts=2):
        sd_rec = self.nusc.get('sample', sample_token)
        ...
        #         pose_mat = get_global_sensor_pose(sample_cur, self.nusc, inverse=False)
        #         ego_his_trajs[i] = pose_mat[:3, 3]
        #         has_prev = sample_cur['prev'] != ''
        #         has_next = sample_cur['next'] != ''
        #         if has_next:
        #             sample_next = self.nusc.get('sample', sample_cur['next'])
        #             pose_mat_next = get_global_sensor_pose(sample_next, self.nusc, inverse=False)
        #             ego_his_trajs_diff[i] = pose_mat_next[:3, 3] - ego_his_trajs[i]
        #         sample_cur = self.nusc.get('sample', sample_cur['prev']) if has_prev else None
        #     else:
